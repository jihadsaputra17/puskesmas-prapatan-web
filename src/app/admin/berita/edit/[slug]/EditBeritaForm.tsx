...
export default function EditBeritaForm({ berita }: { berita: Berita }) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    try {
      const r = await fetch(`/api/berita/${berita.id}`, { method: "PUT", body: form });
      const result = await r.json().catch(() => ({}));
      if (!r.ok || result.error) {
        setFields(result.fields || {});
        setFeedback({ type: "error", message: result.error || "Gagal memperbarui berita." });
      } else {
  ...
  return (
    <div className="panel mx-auto max-w-3xl p-6 sm:p-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3 border-b border-[var(--line)] pb-4">
        <div>
          <h1 className="text-2xl font-bold text-[var(--navy)]">Edit berita</h1>
          <p className="mt-1 text-sm text-slate-600">
            Perubahan langsung memakai layout majalah di{" "}
            <Link
              href={`/berita/${berita.slug || ""}`}
              className="font-semibold text-clinic-teal hover:underline"
              target="_blank"
      ...
      <form onSubmit={submit} className="space-y-8">
        <BeritaFormFields
          values={{
            ...
            template: berita.template || "standard",
          }}
          fields={fields}
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-[var(--line)] pt-6">
          ...
          {berita.slug ? (
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-sm font-semibold text-clinic-teal hover:underline"
            >
              Lihat di situs ↗
            </Link>
          ) : null}
