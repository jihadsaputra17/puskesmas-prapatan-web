...
function readFieldErrors(data: unknown): FieldErrors {
  ...
  const errors = (data as { fieldErrors?: unknown }).fieldErrors;
...
export default function AddUserForm() {
  ...
  return <div className="max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
    ...
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
      <Field label="Nama Lengkap" name="name" error={fieldErrors.name}><input type="text" id="name" name="name" required placeholder="Contoh: Budi Santoso" className="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-600" /></Field>
      <Field label="Email" name="email" error={fieldErrors.email}><input type="email" id="email" name="email" required placeholder="contoh@puskesmas.com" className="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-600" /></Field>
      <Field label="Kata Sandi" name="password" error={fieldErrors.password}><input type="password" id="password" name="password" required minLength={8} placeholder="Minimal 8 karakter" className="w-full rounded-lg border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-600" /></Field>
      <Field label="Peran" name="role" error={fieldErrors.role}><select id="role" name="role" required defaultValue="admin" className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-600"><option value="admin">Admin</option><option value="superadmin">Superadmin</option></select></Field>
